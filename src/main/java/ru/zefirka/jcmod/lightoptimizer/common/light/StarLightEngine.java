...
public abstract class StarLightEngine {
    ...
    protected final void setupCaches(final IChunkLightProvider chunkProvider, final int centerX, final int centerY, final int centerZ,
        ...
        for (int dz = -radius; dz <= radius; ++dz) {
            for (int dx = -radius; dx <= radius; ++dx) {
                ...
                final IChunk chunk = (IChunk)chunkProvider.getChunkForLighting(cx, cz); // mappings are awful here, this is the "get chunk at if at least features"
    ...
    protected final BlockState getBlockState(final int sectionIndex, final int localIndex) {
        ...
        if (section != null) {
            return section == EMPTY_CHUNK_SECTION ? AIR_BLOCK_STATE : section.getStates().get(localIndex);
        }
    ...
    protected abstract void setEmptinessMap(final IChunk chunk, final boolean[] to);
    ...
    protected final boolean[] handleEmptySectionChanges(final IChunkLightProvider lightAccess, final IChunk chunk,
                                                        final Boolean[] emptinessChanges, final boolean unlit) {
        final World world = (World)lightAccess.getLevel();
        final int chunkX = chunk.getPos().x;
        final int chunkZ = chunk.getPos().z;
    ...
    public final void relightChunks(final IChunkLightProvider lightAccess, final Set<ChunkPos> chunks,
        ...
        for (final ChunkPos chunkPos : chunks) {
            final int chunkX = chunkPos.x;
            final int chunkZ = chunkPos.z;
            final IChunk chunk = (IChunk)lightAccess.getChunkForLighting(chunkX, chunkZ);
            ...
            for (int i = 0, len = neighbourLightOrder.length; i < len; i += 2) {
                ...
                final IChunk neighbour = (IChunk)lightAccess.getChunkForLighting(neighbourX, neighbourZ);
                ...
                try {
                    ...
                    for (int dz2 = -1; dz2 <= 1; ++dz2) {
                        for (int dx2 = -1; dx2 <= 1; ++dx2) {
                            ...
                            final IChunk neighbour2 = (IChunk)lightAccess.getChunkForLighting(neighbourX2, neighbourZ2);
            ...
            for (int y = this.minLightSection; y <= this.maxLightSection; ++y) {
                lightAccess.onLightUpdate(this.skylightPropagator ? LightType.SKY : LightType.BLOCK, SectionPos.of(chunkX, y, chunkX));
            }
    ...
    protected final void performLightIncrease(final IChunkLightProvider lightAccess) {
        final IBlockReader world = lightAccess.getLevel();
        long[] queue = this.increaseQueue;
        ...
        while (queueReadIndex < queueLength) {
            ...
            if ((queueValue & FLAG_HAS_SIDED_TRANSPARENT_BLOCKS) == 0L) {
                ...
                for (final AxisDirection propagate : checkDirections) {
                    ...
                    if (opacityCached != -1) {
                        ...
                        if (((ExtendedAbstractBlockState)blockState).isConditionallyFullOpaque()) {
                            ...
                            if (VoxelShapes.faceShapeOccludes(VoxelShapes.empty(), cullingFace)) {
                                continue;
                            }
                ...
                for (final AxisDirection propagate : checkDirections) {
                    ...
                    if (fromShape != VoxelShapes.empty() && VoxelShapes.faceShapeOccludes(VoxelShapes.empty(), fromShape)) {
                        continue;
                    }
                    ...
                    if (opacityCached != -1) {
                        ...
                        if (((ExtendedAbstractBlockState)blockState).isConditionallyFullOpaque()) {
                            ...
                            if (VoxelShapes.faceShapeOccludes(fromShape, cullingFace)) {
                                continue;
                            }
                            flags |= FLAG_HAS_SIDED_TRANSPARENT_BLOCKS;
                        }
